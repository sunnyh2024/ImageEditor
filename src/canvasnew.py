...
class PaintScene(QGraphicsScene):
    ...
    canvasEdited = pyqtSignal(QPainterPath)
    ...
    def start_paintstroke(self, position, layer=None):
        ...
        opacity.setOpacity(self.pen_opacity)
        self._current_path.setGraphicsEffect(effect)
        self._current_path.setGraphicsEffect(opacity)
        ...
        preview_effect = QGraphicsBlurEffect()
    ...
    def set_pen_opacity(self, opacity):
        ...
        Args:
            opacity (int): new pen opacity (0-255)
        """
...
class PaintView(QGraphicsView):
    ...
    def mouseReleaseEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            scene_pos = self.mapToScene(event.pos())
            stroke = self.scene().complete_paintstroke(scene_pos)
